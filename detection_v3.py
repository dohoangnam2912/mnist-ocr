...
def preprocess_roi(roi):
    """
    Preprocess the ROI by centering, padding, and resizing for uniformity.
    """
    # Ensure the image is in grayscale
    if len(roi.shape) == 3:
        roi = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)

    # Apply binary thresholding for clear edges
    ...
    # Add padding to center the character within a 28x28 frame
    top = (28 - new_h) // 2
    bottom = 28 - new_h - top
    left = (28 - new_w) // 2
    right = 28 - new_w - left
    ...
    roi = roi.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions
...
def detect_and_predict_text(image_path, model):
    ...
    # Segment lines and add red boxes for each line
    ...
    for line_start, line_end in lines:
        ...
        for word_start, word_end in words:
            ...
            plt.title(f'Original Word Image {word_counter}')
            plt.axis('off')

            # Preprocess the entire word_image as the input to the model
            roi = preprocess_roi(word_image)

            # Display the preprocessed ROI
            plt.subplot(1, 2, 2)
            plt.imshow(roi.squeeze(), cmap='gray')
            plt.title(f'Preprocessed ROI {word_counter}')
            plt.axis('off')
            plt.show()

            # Make prediction for the entire word
            with torch.no_grad():
                output = model(roi)
                ...
                if pred_index < len(characters):
                    predicted_word = characters[pred_index]
                else:
                    predicted_word = "?"  # Placeholder for out-of-range predictions

            # Append the predicted word to the line text
    ...
    plt.title("Detected Lines and Words")
    plt.axis('off')
    plt.show()
